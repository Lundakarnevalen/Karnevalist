...
class SuperAgileAlert extends Component {
  ...
  createSingleButton(index) {
    ...
    const length = buttonsIn.length
    const { buttonWidth } = this.state;
    if (buttonsIn.length === 1) {
      return (
        <CustomButton
          onPress={buttonsIn[0].onPress}
          text={buttonsIn[0].text}
          style={'alertButton'}
          width={buttonWidth}
        />
      );
    ...
    if (index === 0) {
      return (
        <CustomButton
          ...
          style={'alertButton'}
          width={Dimensions.get('window').width / (1.1 * length)}
        />
      );
    }
          ...
          <View style={styles.outerViewStyle} transparent={false}>
            <View style={styles.alertBoxStyle}>
              ...
              <View style={{ position: 'absolute', bottom: 0, flexDirection: 'row' }}>
                { this.props.buttonsIn.map((button, i) => {
                  return this.createSingleButton(i)
                })
                }
              </View>
            </View>
