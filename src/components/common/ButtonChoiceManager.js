...
class ButtonChoiceManager extends Component {
  ...
  whenPressedCheckBox(id) {
    ...
    if (!this.contains(markedButtons, id)) {
      ...
      this.setState({
        markedButtons: markedButtons.filter(markedButtonID => markedButtonID !== id)
      });
    }
    console.log(markedButtons)
  }

  whenPressedRadio(id) {
    const { markedButtons } = this.state;
    if (!this.contains(markedButtons, id) && markedButtons.length === 0) {
      const newMarkedButtons = markedButtons;
      newMarkedButtons.push(id);
      this.setState({ markedButtons: newMarkedButtons });
  ...
  render() {
    return (
      <View style={{ flexDirection: this.props.alignment || 'row' }}>{this.addButtons()}</View>
    );
  }
}
